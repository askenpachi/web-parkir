...
function getVehicleChartData() {
  const data = vehicleFilter.value === 'daily' ? [5, 10, 5]
    : vehicleFilter.value === 'weekly' ? [20, 30, 25]
    : [200, 300, 250];
...
function updateVehicleChart() {
  if (vehicleChart) {
    vehicleChart.data = getVehicleChartData();
    vehicleChart.update();
  }
}
</script>
